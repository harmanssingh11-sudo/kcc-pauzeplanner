...
// V5.11: ondersteunt dagtypes KCC/Webcare/Balie. Balie-medewerkers mogen onderling nooit
// gelijktijdig pauze hebben. Een dagtype overrulet het algemene profieltype.
const DAYS=['Ma','Di','Wo','Do','Vr','Za','Zo'];
const BIG_SLOTS=['12:00','12:35','13:10','13:45','14:20','14:55','15:30','16:00'];
const BIG_DUR=30, MINI_DUR=10, NORMAL_CAP=2, EXCEPTION_CAP=3, MINI_CUTOFF=960, PAUSE_BUFFER=5;
...
function isEligible(p,date){if(!p.active)return false;const s=scheduleFor(p,date);if(!s?.work)return false;return ['KCC','Balie'].includes(effectiveType(p,s))&&toMin(s.end)-toMin(s.start)>240}
...
function balieConflict(plan,p,t,dur){const type=(p._planningType||p.type);if(type!=='Balie')return false;return plan.some(b=>b.p!==p&&(b.p._planningType||b.p.type)==='Balie'&&overlaps(t,dur,b.t,b.kind==='big'?BIG_DUR:MINI_DUR))}
...
function validMiniTimes(plan,times,p){return times.filter(t=>!balieConflict(plan,p,t,MINI_DUR)&&respectsMiniBuffer(plan,t))}
function buildPlan(people,date,bias={},enabled={}){
 ...
 elig.forEach(p=>p._planningType=effectiveType(p,scheduleFor(p,date)));
 const plan=[],warnings=[];const wants=elig.filter(p=>on('big')&&rightsFor(scheduleFor(p,date)).includes('big'));const pref=wants.filter(p=>p.pref),normal=wants.filter(p=>!p.pref);
 const addBig=(p,isPref)=>{const s=scheduleFor(p,date),allowed=FIXED_SLOTS.filter(t=>t>=Math.max(720,toMin(s.start))&&t+BIG_DUR<=toMin(s.end));if(!allowed.length){warnings.push(`${p.name}: geen grote pauze binnen dienstvenster.`);return}const base=isPref?toMin(p.pref):toMin(s.start)+240;const wanted=base;const ordered=[...allowed].sort((a,b)=>Math.abs(a-wanted)-Math.abs(b-wanted));let t=ordered.find(x=>!balieConflict(plan,p,x,BIG_DUR)&&bigOcc(plan,x)<NORMAL_CAP),exception=false;if(t===undefined)t=ordered.find(x=>!balieConflict(plan,p,x,BIG_DUR)&&bigOcc(plan,x)<EXCEPTION_CAP),exception=true;if(t===undefined)t=ordered.find(x=>!balieConflict(plan,p,x,BIG_DUR)),exception=true;if(t===undefined){warnings.push(`${p.name}: geen grote pauze beschikbaar zonder gelijktijdige Balie-pauze.`);return}if(exception&&!isPref)warnings.push(`${p.name}: 3e grote pauze tegelijk om ${toHHMM(t)}.`);if(isPref&&t!==base)warnings.push(`${p.name}: voorkeur kon niet exact binnen dienst worden geplaatst.`);plan.push({p,kind:'big',t,pref:isPref,exception})};
 pref.forEach(p=>addBig(p,true));normal.forEach(p=>addBig(p,false));
 ['mini1','mini2'].forEach(kind=>{if(!on(kind))return;const candidates=[];elig.forEach(p=>{const s=scheduleFor(p,date),r=rightsFor(s);if(!r.includes(kind))return;const big=plan.find(b=>b.p===p&&b.kind==='big'),m1=plan.find(b=>b.p===p&&b.kind==='mini1');let lo,hi,target;if(kind==='mini1'){const [a,b]=mini1Window(s);lo=Math.max(a,toMin(s.start));hi=Math.min(b,toMin(s.end)-MINI_DUR,MINI_CUTOFF-MINI_DUR);target=big?big.t-120:lo}else{lo=Math.max(toMin(s.start)+60,big?big.t+30:m1?m1.t+MINI_DUR:toMin(s.start));hi=Math.min(toMin(s.end)-MINI_DUR,MINI_CUTOFF-MINI_DUR);target=big?big.t+120:m1?m1.t+120:toMin(s.start)+240}hi=Math.min(hi,MINI_CUTOFF-MINI_DUR);if(lo<=hi){target+=bias[p.id+'|'+kind]||0;target=Math.max(lo,Math.min(hi,target));const times=[];for(let t=lo;t<=hi;t+=5)times.push(t);candidates.push({p,kind,times,target,flex:times.length})}else warnings.push(`${p.name}: ${kind==='mini1'?'Mini 1':'Mini 2'} past niet vóór 16:00 en wordt niet na 16:00 gepland.`)});
  const unscheduled=new Set(candidates.map(c=>c.p.id));const place=(c,t,exception)=>{plan.push({p:c.p,kind:c.kind,t,pref:false,exception:!!exception});unscheduled.delete(c.p.id)};
  while(unscheduled.size){let best=null;for(const c of candidates){if(!unscheduled.has(c.p.id))continue;const pairTimes=c.times.filter(t=>!balieConflict(plan,c.p,t,MINI_DUR)&&plan.some(b=>b.kind!=='big'&&b.t===t)&&miniOcc(plan,t)===1);if(!pairTimes.length)continue;pairTimes.sort((a,b)=>Math.abs(a-c.target)-Math.abs(b-c.target));const optionCount=pairTimes.length;if(!best||optionCount<best.optionCount||(optionCount===best.optionCount&&Math.abs(pairTimes[0]-c.target)<Math.abs(best.t-best.c.target)))best={c,t:pairTimes[0],optionCount}}if(best){place(best.c,best.t,false);continue}
   let single=null;for(const c of candidates){if(!unscheduled.has(c.p.id))continue;const zero=validMiniTimes(plan,c.times,c.p).filter(t=>miniOcc(plan,t)===0).sort((a,b)=>a-b);if(!zero.length)continue;if(!single||c.flex<single.c.flex||(c.flex===single.c.flex&&zero[0]<single.t))single={c,t:zero[0]}}if(single){place(single.c,single.t,false);continue}break}
  for(const c of candidates){if(!unscheduled.has(c.p.id))continue;const three=c.times.filter(t=>!balieConflict(plan,c.p,t,MINI_DUR)&&plan.some(b=>b.kind!=='big'&&b.t===t)&&miniOcc(plan,t)===2).sort((a,b)=>a-b);if(three.length){place(c,three[0],true);warnings.push(`${c.p.name}: 3e mini tegelijk om ${toHHMM(three[0])}; alleen gebruikt omdat geen duo/single-plek meer beschikbaar was.`)}else warnings.push(`${c.p.name}: geen mini-capaciteit beschikbaar binnen de toegestane tijden.`)}
 });
