...
// V5.7: Mini 1 target starts at earliest valid point to avoid unnecessary gaps; duo/triple exact start; solo 5-min buffer.
...
function buildPlan(people,date){
 ...
 ['mini1','mini2'].forEach(kind=>{const candidates=[];elig.forEach(p=>{const s=scheduleFor(p,date),r=rightsFor(s);if(!r.includes(kind))return;const big=plan.find(b=>b.p===p&&b.kind==='big'),m1=plan.find(b=>b.p===p&&b.kind==='mini1');let lo,hi,target;if(kind==='mini1'){const [a,b]=mini1Window(s);lo=Math.max(a,toMin(s.start));hi=Math.min(b,toMin(s.end)-MINI_DUR,MINI_CUTOFF-MINI_DUR);target=big?big.t-120:lo}else{lo=Math.max(toMin(s.start)+60,big?big.t+30:m1?m1.t+MINI_DUR:toMin(s.start));hi=Math.min(toMin(s.end)-MINI_DUR,MINI_CUTOFF-MINI_DUR);target=big?big.t+120:m1?m1.t+120:toMin(s.start)+240}hi=Math.min(hi,MINI_CUTOFF-MINI_DUR);if(lo<=hi){const times=[];for(let t=lo;t<=hi;t+=5)times.push(t);candidates.push({p,kind,times,target,flex:times.length})}else warnings.push(`${p.name}: ${kind==='mini1'?'Mini 1':'Mini 2'} past niet vóór 16:00 en wordt niet na 16:00 gepland.`)});
