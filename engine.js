...
// V5 FIX: mini pauses finish by 16:00; pair first, 3 only as fallback.
...
function buildPlan(people,date){
 ...
 const addBig=(p,isPref)=>{
  const s=scheduleFor(p,date);
  const allowed=FIXED_SLOTS.filter(t=>t>=Math.max(720,toMin(s.start))&&t+BIG_DUR<=toMin(s.end));
  if(!allowed.length){warnings.push(`${p.name}: geen grote pauze binnen dienstvenster.`);return}
 ...
 pref.forEach(p=>addBig(p,true));
 normal.forEach(p=>addBig(p,false));

 // Build all legal mini candidates first. This lets us deliberately pack pairs
 // before opening a third position instead of greedily scattering single people.
 const miniJobs=[];
 ['mini1','mini2'].forEach(kind=>{
  elig.forEach(p=>{
   const s=scheduleFor(p,date),r=rightsFor(s);
   if(!r.includes(kind))return;
   const big=plan.find(b=>b.p===p&&b.kind==='big');
   const m1=plan.find(b=>b.p===p&&b.kind==='mini1');
   let lo,hi,target;
   if(kind==='mini1'){
   ...
   const candidates=[];
   for(let t=lo;t<=hi;t+=5)candidates.push(t);
   miniJobs.push({p,kind,candidates,target});
  });
 });

 // Tightest jobs first. For each job we first look for a slot with exactly
 // one mini already present (creating a pair). Only when no pair is possible
 // do we open an empty slot. A slot with two is considered only as the final
 // fallback, i.e. the 3-person exception.
 miniJobs.sort((a,b)=>a.candidates.length-b.candidates.length||a.target-b.target||a.p.name.localeCompare(b.p.name));
 for(const job of miniJobs){
  const pair=job.candidates.filter(t=>miniOcc(plan,t)===1);
  const empty=job.candidates.filter(t=>miniOcc(plan,t)===0);
  const triple=job.candidates.filter(t=>miniOcc(plan,t)===2);
  const rank=(arr)=>[...arr].sort((a,b)=>Math.abs(a-job.target)-Math.abs(b-job.target)||totalLoad(plan,a)-totalLoad(plan,b));
  let chosen;
  let mode='pair';
  if(pair.length)chosen=rank(pair)[0];
  else if(empty.length){chosen=rank(empty)[0];mode='new';}
  else if(triple.length){chosen=rank(triple)[0];mode='triple';}
  else{
   warnings.push(`${job.p.name}: geen mini-capaciteit beschikbaar binnen het toegestane venster.`);
   continue;
  }
  if(chosen+MINI_DUR>MINI_CUTOFF){
   warnings.push(`${job.p.name}: mini zou na 16:00 eindigen en is daarom niet ingepland.`);
   continue;
  }
  if(mode==='triple')warnings.push(`${job.p.name}: 3e mini tegelijk om ${toHHMM(chosen)}; alleen gebruikt omdat geen 1-op-1 combinatie mogelijk was.`);
  plan.push({p:job.p,kind:job.kind,t:chosen,pref:false,exception:mode==='triple'});
 }

 // Final safety filter: no late mini can reach the UI.
 for(let i=plan.length-1;i>=0;i--){
  if(plan[i].kind!=='big'&&plan[i].t+MINI_DUR>MINI_CUTOFF){
   warnings.push(`${plan[i].p.name}: late mini verwijderd; mini's moeten uiterlijk 16:00 eindigen.`);
   plan.splice(i,1);
  }
 }
